...
For the session ID (the `claude --resume <id>` fallback), take the newest transcript
in the project's history directory:

```bash
ls -t ~/.claude/projects/"$(pwd | sed 's|/|-|g')"/*.jsonl 2>/dev/null | head -1
```

This is the most-recent-transcript heuristic, not a self-identification — say so in
the doc rather than presenting the ID as certain.
...
3. **Respect the repo's invariants.** Read `CLAUDE.md`, `ARCHITECTURE.md`, and
