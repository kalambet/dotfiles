...
**Platform conventions are law.** Apple has spent decades refining interaction patterns. Fighting the platform creates fragile, surprising UIs that break with every OS update. If Apple has a pattern for it — navigation, tab bars, sheets, context menus, menus — use Apple's pattern. Native-first is the baseline, not a preference: a native control inherits system metrics, highlight, focus, keyboard navigation, and VoiceOver for free. A custom control that replaces a native element is a finding unless the deviation was a deliberate, operator-approved choice that is clearly stated in the code or PR — and even then it must fully reimplement the accessibility and keyboard behavior it gave up. Flag any custom reimplementation of a native element where the native path wasn't tried, and any silent deviation no one signed off on.
