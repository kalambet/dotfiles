---
name: rust-team
description: Rust Team — architect designs the issue's implementation, developer judges the design from the code standpoint and implements after consensus, reviewer judges the result as a pair of fresh eyes with no design context. Use when the user asks to build, implement, or fix something non-trivial in Rust with the team, e.g. "get the rust team on this", "rust team: implement X", or any substantial Rust change that deserves a design-consensus-review cycle. Not for trivial one-line Rust fixes — delegate those to rust-developer directly.
---
...
|---|---|---|
| `architect` | `rust-architect` | Designs the implementation for this issue |
| `developer` | `rust-developer` | Critiques the design from the code standpoint, then implements the consensus |
| `reviewer` | `rust-reviewer` | Reviews the diff cold — fresh eyes, no design context |

Each agent preloads its skill (`rust-architect` / `rust-developer` /
`rust-reviewer`), so the conventions travel with them.
...
relevant crates/modules (paths), acceptance criteria, and constraints (MSRV,
runtime, public-API stability, performance budgets). State assumptions explicitly
...
`team/design.md`: the recommended implementation shape — module/crate placement,
public surface, error types, async commitments, unsafe posture (if any, with the
soundness argument), and the test plan — plus rejected alternatives with reasons.
Design only; no implementation code beyond signatures and skeletons.
...
- `OBJECT` — the decision creates a real implementation problem: names the
  mechanism (borrow-checker fight, unnecessary trait indirection, async boundary in
  the wrong place, missing error path), not a taste complaint.
...
matches the surrounding code, writes tests with the change, runs
`cargo fmt --check`, `cargo clippy --all-targets -- -D warnings`, and `cargo test`,
and reports results honestly. Deviations from the consensus that the code forces
get recorded in `team/design.md`'s decision log — silent drift is what Phase 4
exists to catch.
...
the point: the reviewer judges whether the code stands on its own — naming,
structure, SAFETY comments, tests — exactly as a stranger reading the PR would.

The reviewer works its checklist, runs the same gates, and returns findings with a
verdict: LGTM / LGTM-with-nits / request-changes / refuse-to-LGTM.
...
- Still contested at the cap → present both positions to the user as open
  decisions.
- A refuse-to-LGTM on `unsafe` without a SAFETY justification is never overridden
  by you — only the user can accept that.
...
2. Tests are implemented and green (`fmt`/`clippy`/`test` gates pass).
...
`team/design.md`, gate results (fmt/clippy/test), the reviewer's final verdict,
and any open decisions with both positions.
...
design decisions), say so and delegate straight to `rust-developer` with a
follow-up `rust-reviewer` pass — don't run the full protocol for a rename.
